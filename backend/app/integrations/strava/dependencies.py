...
from app.integrations.strava.client import StravaClient, StravaOAuthClient
from app.integrations.strava.synchronization import StravaSynchronizationService
...
def get_strava_synchronization_repository(
    session: Annotated[Session, Depends(get_db_session)],
) -> StravaSynchronizationRepository:
    return StravaSynchronizationRepository(session)


def get_strava_synchronization_service(
    token_service: Annotated[
        StravaTokenService, Depends(get_strava_token_service)
    ],
    client: Annotated[StravaClient, Depends(get_strava_client)],
    repository: Annotated[
        StravaSynchronizationRepository,
        Depends(get_strava_synchronization_repository),
    ],
